...
export function Select({ value, onChange, options, className, placeholder }: SelectProps) {
  ...
  return (
    <div ref={ref} className={cn('relative', className)}>
      ...
      {open && (
        <div className="absolute z-50 top-full left-0 mt-1 min-w-full w-max glass-card rounded-xl p-1 shadow-2xl">
          {options.map((opt) => {
              ...
              <button
                ...
                className={cn(
                  ...
                  active
                    ...
                    : 'text-foreground/70 hover:bg-overlay/[0.06] hover:text-foreground'
                )}
                onClick={() => {
                  onChange(opt.value)
                  setOpen(false)
